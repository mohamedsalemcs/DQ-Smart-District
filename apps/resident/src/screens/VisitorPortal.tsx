      ...
      <header className="bg-ink-0 px-4 py-5 text-ink-800">
        <div className="mx-auto flex max-w-lg items-center gap-3">
          ...
          <div>
            ...
            <p className="text-caption text-ink-500">بوابة الزوار — تصريح دخول يومي</p>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-lg px-4">
        ...
        {done ? (
          <div className="mt-4 rounded-card bg-ink-0 p-6 text-center">
            ...
            <div className="mt-4 rounded-card bg-ink-50 p-3 text-start text-caption">
          ...
          <div className="mt-4 space-y-4">
              ...
              <div className="mb-3 flex gap-1.5">
                {[0, 1, 2].map((o) => (
                  <button
                    ...
                    className={`flex-1 rounded-card border p-2 text-center text-caption ${dayOffset === o ? 'border-brand-500 bg-brand-50 font-bold' : 'border-ink-100'}`}
                  >
                    <p className="font-semibold">{o === 0 ? 'اليوم' : o === 1 ? 'غدًا' : format(addDays(new Date(), 2), 'EEEE', { locale: ar })}</p>
                    <p className="text-micro text-ink-500">{format(addDays(new Date(), o), 'd MMM', { locale: ar })}</p>
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-card bg-ink-0 p-5">
              ...
              {err && <p className="mt-3 rounded-card border border-danger-500 bg-danger-50 p-2.5 text-caption text-danger-600">{err}</p>}
              ...
              <p className="mt-2 text-center text-micro text-ink-500">دفع تجريبي — محاكاة بوابة سداد داخل النموذج</p>
            </div>
          </div>
        )}
      </main>

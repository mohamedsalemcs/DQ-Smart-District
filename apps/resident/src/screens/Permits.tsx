          ...
          <Card key={p.id} className="p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-[--text-caption] text-ink-500">{permitKindAr[p.kind]} · مرافقون: {p.companions}</p>
                {p.plate && <div className="mt-1.5"><PlateBadge plate={p.plate} size="sm" /></div>}
                <p className="mt-1.5 text-[--text-caption] tabular-nums text-ink-500">
                  {fmtDateTime(p.validFromISO)} ← {fmtDateTime(p.validToISO)}
                </p>
                {p.approvals.some((a) => a.decision === 'info_requested') && p.status === 'pending' && (
                  <p className="mt-1.5 rounded bg-warn-600-50 px-2 py-1 text-[--text-caption]">الإدارة طلبت معلومات إضافية</p>
                )}
              </div>
              <div className="flex flex-col items-center gap-2">
                {permitPill(p.status)}
                {p.status === 'approved' && (
                  <div className="rounded-[--radius-card] border border-ink-100 p-2 text-center">
                    <QrCode size={44} className="mx-auto text-ink-900" />
                    <bdi dir="ltr" className="plate mt-0.5 block text-[--text-micro] text-ink-500">{p.qrToken}</bdi>
                  </div>
                )}
              </div>
            </div>

...
export function Community() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      {featured && (
        <div className="relative overflow-hidden rounded-[--radius-card] bg-ink-0 p-5 text-ink-800 ring-1 ring-brand-500/40">
          ...
          <div className="relative">
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1 rounded-full bg-brand-600 px-2 py-0.5 text-[--text-micro] font-bold text-ink-900">
                <Sparkles size={10} /> إعلان مميز
              </span>
              <span className="text-[--text-caption] text-ink-500">{advertiserOf(featured.advertiserPropId)?.subtypeAr}</span>
            </div>
        ...
        <div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {rest.map((ad) => {
              ...
              return (
                <div key={ad.id} className="flex flex-col overflow-hidden rounded-[--radius-card] bg-ink-0 ring-1 ring-ink-100">
                  ...
                  <div className="flex flex-1 flex-col p-4">
                    <div className="flex items-center justify-between gap-2">
                      <span className="flex items-center gap-2">
                        ...
                        <span className="text-[--text-caption] font-semibold text-ink-500">{advertiser?.subtypeAr}</span>
                      </span>
                      <span className="shrink-0 rounded-full bg-ink-50 px-1.5 py-0.5 text-[--text-micro] text-ink-500">إعلان</span>
                    </div>
                    <p className="mt-3 text-sm font-bold">{ad.titleAr}</p>
                    <p className="mt-1 flex-1 text-[--text-caption] leading-relaxed text-ink-500">{ad.bodyAr}</p>
                    <button
                      onClick={() => interested(ad.titleAr)}
                      className="mt-3 self-start rounded-[--radius-ctl] px-3 py-1.5 text-[--text-caption] font-bold text-white transition-transform hover:-translate-y-0.5"
        ...
        <div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {upcoming.map((b) => {
              ...
              return (
                <div key={b.id} className="rounded-[--radius-card] bg-gradient-to-b from-tint to-white p-4 text-center ring-1 ring-ink-100">
                  <p className="text-2xl font-bold text-brand-600 tabular-nums">{fmtDate(b.fromISO).split(' ')[0]}</p>
                  <p className="text-[--text-micro] font-semibold text-ink-500">{fmtDate(b.fromISO).split(' ').slice(1).join(' ')}</p>
                  <p className="mt-2 text-sm font-bold">{facility?.nameAr}</p>
                  <p className="mt-1 flex items-center justify-center gap-1 text-[--text-caption] text-ink-500">
      ...
      <div>
        ...
        <div className="space-y-2">
          {NEWS.map((n) => (
            <div key={n.titleAr} className="flex items-center gap-3 rounded-[--radius-card] bg-ink-0 p-3 ring-1 ring-ink-100">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[--radius-card]" style={{ background: n.bg, color: n.color }}>
                <n.icon size={18} />
              </span>
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold">{n.titleAr}</p>
                <p className="text-[--text-caption] text-ink-500">{n.subAr}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-[--radius-card] p-4 text-white" style={{ background: 'linear-gradient(135deg, var(--color-ok-500), var(--color-ok-600))'}} >
          <Sprout size={22} className="opacity-80" />
          <p className="mt-2 text-sm font-bold">مبادرة تشجير الجار</p>
          <p className="mt-1 text-[--text-caption] leading-relaxed opacity-85">وزّعنا 200 شتلة هذا الشهر — سجّل اهتمامك وسيتواصل فريق التشغيل لتخضير محيط وحدتك.</p>
          <button onClick={() => interested('مبادرة تشجير الجار')} className="mt-3 rounded-[--radius-ctl] bg-ink-0/20 px-3 py-1.5 text-[--text-caption] font-bold hover:bg-ink-0/30">
            سجّل اهتمامك
          </button>
        </div>
        <div className="rounded-[--radius-card] p-4 text-ink-900" style={{ background: 'linear-gradient(135deg, var(--color-brand-500), var(--color-brand-300))'}} >
          <Footprints size={22} className="opacity-70" />
          <p className="mt-2 text-sm font-bold">نادي جري الحي الدبلوماسي</p>
          <p className="mt-1 text-[--text-caption] leading-relaxed opacity-80">انطلاقة كل فجر جمعة — نقطة التجمع بوابة حديقة النفل، جميع المستويات مرحّب بها.</p>
          <button onClick={() => interested('نادي جري الحي')} className="mt-3 rounded-[--radius-ctl] bg-ink-0/15 px-3 py-1.5 text-[--text-caption] font-bold hover:bg-ink-0/25">
            انضم للنادي
          </button>
        </div>
      </div>

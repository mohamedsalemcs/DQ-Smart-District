...
function buildHeightGrid(scene: THREE.Object3D, box: THREE.Box3): Float32Array {
  if (heightGridCache) return heightGridCache;
  try {
    const stored = sessionStorage.getItem(GRID_KEY);
    if (stored) {
      heightGridCache = new Float32Array(JSON.parse(stored));
      return heightGridCache;
    }
  } catch { /* ignore */ }

  scene.traverse((o) => {
    const mesh = o as THREE.Mesh;
    if (mesh.isMesh && !(mesh.geometry as any).boundsTree) mesh.geometry.computeBoundsTree?.();
  });
...
function DQModel({ onReady }: { onReady: (p: Projector) => void }) {
  ...
  useEffect(() => {
    ...
    const grid = buildHeightGrid(scene, box);

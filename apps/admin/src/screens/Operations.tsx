  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          <Card className="p-4">
            <p className="mb-2 text-caption font-semibold text-ink-500">تنبيهات حية</p>
            <div className="max-h-56 space-y-1.5 overflow-y-auto thin-scroll">
              {fullBins.map((b) => (
                <p key={b.id} className="flex items-center gap-1.5 rounded bg-danger-50 px-2 py-1.5 text-caption text-danger-600">
                  <Trash2 size={12} /> {b.nameAr} — {fillOf(b.id)}% · صدر أمر عمل
                </p>
              ))}
              {faultPoles.map((p) => (
                <p key={p.id} className="flex items-center gap-1.5 rounded bg-warn-600-50 px-2 py-1.5 text-caption text-warn-600">
                  <Lightbulb size={12} /> {p.nameAr} — عطل
                </p>
              ))}
              {dryTrees.map((t) => (
                <p key={t.id} className="flex items-center gap-1.5 rounded bg-danger-50 px-2 py-1.5 text-caption text-danger-600">
                  <TreePine size={12} /> {t.nameAr} — رطوبة {store.sensorValues[t.id]?.moisture}% · بحاجة ري
                </p>
              ))}
              {lowBattery.slice(0, 4).map((a) => (
                <p key={a.id} className="flex items-center gap-1.5 rounded bg-warn-600-50 px-2 py-1.5 text-caption text-warn-600">
              ...
              {fullBins.length + faultPoles.length + lowBattery.length + dryTrees.length === 0 && (
                <p className="py-2 text-center text-caption text-ink-500">كل الأصول ضمن الحدود الطبيعية</p>
              )}
            </div>
          </Card>

          <Card className="p-4">
            <p className="mb-2 text-caption font-semibold text-ink-500">أعلى الحاويات امتلاءً</p>
            {[...bins]

...
export function DigitalTwin({ }: Record<string, never>) {
  ...
  /* الطبقات الأمنية (البلاغ · البوابة · الدورية · الجولة) تعيش في منصة
     التشغيل الأمني خلف قاعدة نشر أخرى — `go` يعبر إليها بتحميل صفحة كامل،
     ويبقي مسارات الإدارة داخل المسيّر. */
  const open = (link: string) => {
    if (link.startsWith('property:')) return go(`/properties/${link.slice(9)}`);
    if (link.startsWith('incident:')) return go(`/s/incidents/${link.slice(9)}`);
    if (link.startsWith('gate:')) return go(`/s/gate/${link.slice(5)}`);
    if (link === 'patrol') return go('/s/patrol');
    if (link === 'tour') return go('/s/tour');
    if (link === 'operations') return go('/operations');
    if (link === 'requests') return go('/requests');
    if (link === 'violations') return go('/violations');
    go('/');
  };
